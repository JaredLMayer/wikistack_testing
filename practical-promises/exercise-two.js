...
function problemA () {
  ...
  */
  ...
  // promise version
  Promise.all(
      ['poem-two/stanza-01.txt', 'poem-two/stanza-02.txt'].map(function(filename){
        return pReadFile(filename).then(green);
      })
    )
    .then(done);
}
...
function problemC () {
  ...
  */
  ...
  // promise version
  filenames.reduce(function(memo, filename){
...
function problemD () {
  ...
  */
  ...
  filenames.reduce(function(memo, filename){
    ...
    return memo;
  }, Promise.resolve()).then(null, red).then(done);
...
function problemE () {
  ...
  var fs = require('fs');
  function pWriteFile (filename, str) {
  ...
  pWriteFile('foo.txt', 'foo ' + Math.random())
    .then(done);
}
